...
test_ds  = Subset(datasets.ImageFolder(root=data_root, transform=val_transform), val_idx)
...
for epoch in range(1, num_epochs+1):
    ...
    if val_loss < best_val_loss:
        best_val_loss = val_loss
        no_improve_epochs = 0
        torch.save(model.state_dict(), '/home/work/workspace_ai/Artificlass/real_use_code/Log/best_model_from_folder_pre_4_v2.pth')
        print(f"→ New best model saved (val_loss={best_val_loss:.4f})\n")
    else:
        no_improve_epochs += 1
        print(f"  (no improvement for {no_improve_epochs}/{early_stop_patience} epochs)")
...
# 8) JSON 파일로 메트릭 저장
# ─────────────────────────────────────────────────────────────
with open('/home/work/workspace_ai/Artificlass/real_use_code/Log/training_metrics_pre_4_v2.json', 'w') as fp:
    json.dump(metrics, fp, indent=2)

print("✅ Metrics saved to training_metrics_pre_4.json")
