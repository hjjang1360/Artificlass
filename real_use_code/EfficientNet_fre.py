...
# # (1) model 정의, freeze + head optimizer
...
#         scaler.step(optimizer)
#         scaler.update()
#         scheduler.step()         # One-Cycle: 배치마다 호출
#         bs = lbls.size(0)
#         train_loss += loss.item() * bs
#         total      += bs
#     train_loss /= total
...
#     # ────────────────────────────────────────────────
#     # (3) after freeze_epochs, unfreeze + discrim. LR
...
#         no_improve_epochs = 0
#         torch.save(model.state_dict(), 'best_model_efficientnet_b0_freezed.pth')
...
# # 6) JSON로 메트릭 저장
# # ─────────────────────────────────────────────────────────────
# with open('training_metrics_efficientnet_b0_freezed.json', 'w') as fp:
#     json.dump(metrics, fp, indent=2)

# print("✅ Done. Metrics saved to training_metrics_efficientnet_b0_freezed.json")
